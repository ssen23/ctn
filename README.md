...
## 📰 프로젝트 과정
(1). crawler.py를 통해 네이버 뉴스 데이터를 크롤링하고, MongoDB에 저장. 댓글 수·공감 수를 가져오기 위해 Selenium(셀리니움)을 사용.

(2). Lightning.ai 플랫폼 내에서 모델 개발

(3). 초기 개발 단계: 모델 수정의 가능성 존재  →  로컬에서 실시간 모델 돌려서 출력

(3-1). 시스템 안정화 / 배포 시점: 불일치 확률을 MongoDB에 업데이트 → 속도 개선 + 정렬·필터링 용이


#app.py
Flask 웹서버를 구동하고 웹페이지를 렌더링(보여주는 작업)
#db_utils.py
몽고디비(MongoDB) 에 연결해서 뉴스 데이터를 가져옴
#model_utils.py
KoBERT 모델을 로딩(불러오기) 하고, 불일치 확률 등을 계산
